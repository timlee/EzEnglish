...
public class RemindActivity extends Activity {
    public static final String ALARM_ACTION = "com.knowmemo.alarm.clock";
    ...
    //id=鬧鐘id,week=0表示一次性鬧鐘;非0就代表周己的週期性鬧鐘,tips是訊息,flag=0一次性;flag=1每天提醒;flag=2每周幾
    public static void setAlarm(Context context, int flag, int hour, int minute, int id, int
        ...
        if (flag == 0) {
            ...
            intervalMillis = 24 * 3600 * 1000;//每天一次
        } else if (flag == 2) {
            intervalMillis = 24 * 3600 * 1000 * 7;//每周一次
        }
    ...
    //dateTime 傳入時間（設置當天的年月日+選擇框的時分秒）
    private static long calMethod(int weekflag, long dateTime) {
        long time = 0;
        //weekflag == 0表示是一次性的，weekfalg非0時表示每周幾的鬧鐘並以周為時間間隔
        if (weekflag != 0) {
